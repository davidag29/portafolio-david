  ...
  return (
    ...
    >
      ...
      <div className="mt-10 flex w-full max-w-md flex-col gap-3 sm:flex-row">
        <a
          href="#projects"
          className="tech-display flex h-12 flex-1 items-center justify-center border border-[var(--red-active)] bg-[var(--red-root)] px-5 text-sm font-black text-[var(--foreground)] transition hover:bg-[var(--red-active)]"
        >
          {copy.primaryCta} <span className="ml-3">-&gt;</span>
        </a>
        <a
          href="#contact"
          className="tech-display flex h-12 flex-1 items-center justify-center border border-[var(--line-strong)] px-5 text-sm font-black text-[var(--foreground)] transition hover:border-[var(--red-hot)] hover:text-[var(--red-hot)]"
        >
          {copy.secondaryCta} <span className="ml-3">-&gt;</span>
        </a>
      </div>

      <p className="tech-display mt-9 text-xs text-[var(--dim)]">{copy.location}</p>
    </section>
  );
}
